	...
	async (uri, { uid }) => {
		try {
			const response = await axios.get<ContentTypeResponse>(
				`${API_BASE_URL}/content_types/${uid}`,
				{
					headers: getHeaders(),
				},
			);
		...
		options: z
			.object({
				is_page: z
					...
					.optional()
					.describe(
						"Set to true for webpage content types, false for content blocks",
					),
				singleton: z
					.boolean()
					.optional()
				...
				sub_title: z
					.array(z.string())
					.optional()
					.describe("Fields to use as subtitles"),
				url_pattern: z
					.string()
					.optional()
					.describe("Default URL pattern for entries"),
				...
				z.object({
					conditions: z.array(
						z.object({
							operand_field: z
								.string()
								.describe("Field on which to apply condition"),
							operator: z
								.string()
					...
					actions: z.array(
						z.object({
							...
							target_field: z
								.string()
								.describe("Field to show/hide based on condition"),
						}),
					),
					match_type: z
						.string()
						.describe("Whether all or any conditions should be met"),
				}),
			)
			.optional()
			.describe(
				"Field visibility rules for showing/hiding fields based on conditions",
			),
		taxonomies: z
			.array(
				z.object({
					...
					mandatory: z
						.boolean()
						.optional()
						.describe("Whether this taxonomy is required"),
					non_localizable: z
						.boolean()
						.optional()
	...
	async ({ title, uid, schema, options, field_rules, taxonomies }) => {
		try {
			...
			if (field_rules && field_rules.length > 0) {
				payload.content_type.field_rules =
					field_rules as ContentTypeFieldRule[];
			}
			...
			const response = await axios.post<ContentTypeResponse>(
				`${API_BASE_URL}/content_types`,
				payload,
				{
					headers: getHeaders(),
...
server.tool(
	...
	async ({ uid, title, schema, options, field_rules }) => {
		try {
			...
			const fetchResponse = await axios.get<ContentTypeResponse>(
				`${API_BASE_URL}/content_types/${uid}`,
				{
					headers: getHeaders(),
				},
			...
			const payload: ContentTypePayload = {
				content_type: {
					...
					schema: schema
						? (schema as ContentTypeSchema[])
						: existingContentType.schema,
				},
			};
			...
			// Update content type
			const response = await axios.put<ContentTypeResponse>(
				`${API_BASE_URL}/content_types/${uid}`,
				payload,
				{
					headers: getHeaders(),
...
server.tool(
	...
	"Retrieves a specific entry by its content type UID and entry UID, with options for locale and including references.",
	{
		...
		include_references: z
			.array(z.string())
			.optional()
			.describe("References to include"),
		include_reference_content_type_uid: z
			.boolean()
			.optional()
			.default(false)
...
server.tool(
	...
	"Retrieves entries for a specified content type, with extensive options for filtering, sorting, pagination, and including related data.",
	{
		content_type_uid: z
			.string()
			.describe("Content type UID to fetch entries from"),
		locale: z.string().optional().describe("Locale code (e.g., 'en-us')"),
		query: z
			.string()
			.optional()
			.describe("Query in JSON format to filter entries"),
		include_count: z
			.boolean()
			.optional()
			.default(false)
...
server.tool(
	...
	"Publishes an entry to a specified environment and locale.",
	{
		...
		locale: z
			.string()
			.default("en-us")
			.describe("Locale code (defaults to en-us)"),
	},
...
server.tool(
	...
	async ({ title, uid, schema }) => {
		try {
			...
			console.log(
				"Sending global field payload:",
				JSON.stringify(payload, null, 2),
			);

			const response = await axios.post<GlobalFieldResponse>(
				`${API_BASE_URL}/global_fields`,
				payload,
				{
					headers: getHeaders(),
				},
...
server.tool(
	...
	async ({ uid, title, schema }) => {
		try {
			...
			const fetchResponse = await axios.get<GlobalFieldResponse>(
				`${API_BASE_URL}/global_fields/${uid}`,
				{
					headers: getHeaders(),
				},
			);
			...
			// Update global field
			const response = await axios.put<GlobalFieldResponse>(
				`${API_BASE_URL}/global_fields/${uid}`,
				payload,
				{
					headers: getHeaders(),
