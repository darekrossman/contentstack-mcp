...
const server = new McpServer({
  ...
  version: '0.1.0',
  description: "Integrates with Contentstack's Content Management API for content type and entry management.",
})
...
server.resource(
  ...
  async (uri) => {
    try {
      const response = await axios.get<ContentTypesResponse>(`${API_BASE_URL}/content_types`, {
        headers: getHeaders(),
      })
...
server.resource(
  ...
  async (uri, { uid }) => {
    try {
      const response = await axios.get<ContentTypeResponse>(`${API_BASE_URL}/content_types/${uid}`, {
        headers: getHeaders(),
      })
...
server.resource(
  ...
  async (uri, { content_type_uid }) => {
    try {
      const response = await axios.get<EntriesResponse>(`${API_BASE_URL}/content_types/${content_type_uid}/entries`, {
        headers: getHeaders(),
      })
...
server.tool(
  ...
  {
    include_global_field_schema: z.boolean().optional().default(false).describe('Include global field schemas'),
    include_count: z.boolean().optional().default(false).describe('Include total count of content types'),
    skip: z.number().optional().default(0).describe('Number of content types to skip (for pagination)'),
    limit: z.number().optional().default(100).describe('Number of content types to return (max 100)'),
  ...
  async ({
    ...
    try {
      ...
      const response = await axios.get<ContentTypesResponse>(url.toString(), {
        headers: getHeaders(),
      })
      ...
      if (response.data.content_types.length > 0) {
        ...
        if (response.data.content_types.length > 10) {
          ...
          formattedResponse += `\n(${response.data.content_types.length - 10} more content types not shown)\n\n`
        } else {
          formattedResponse += 'Content types:\n'
          response.data.content_types.forEach((contentType, index) => {
            formattedResponse += `${index + 1}. ${contentType.uid} - ${contentType.title}\n`
          })
          formattedResponse += '\n'
        }
      ...
      return {
        content: [
          {
            type: 'text',
            text: `Error retrieving content types: ${handleError(error as ApiError)}`,
          },
        ],
        isError: true,
      }
...
server.tool(
  ...
  async ({ uid, title, schema }) => {
    try {
      ...
      const fetchResponse = await axios.get<GlobalFieldResponse>(`${API_BASE_URL}/global_fields/${uid}`, {
        headers: getHeaders(),
      })
      ...
      if (schema) {
        payload.global_field.schema = schema as ContentTypeSchema[]
      }

      // Update global field
      const response = await axios.put<GlobalFieldResponse>(`${API_BASE_URL}/global_fields/${uid}`, payload, {
        headers: getHeaders(),
      })
...
server.prompt(
  ...
  {
    content_type_uid: z.string().describe('Content type UID for which you want to create content'),
  },
  async ({ content_type_uid }) => {
    try {
      // Fetch content type details to get structure
      const response = await axios.get<ContentTypeResponse>(`${API_BASE_URL}/content_types/${content_type_uid}`, {
        headers: getHeaders(),
      })
...
server.prompt(
  ...
  async ({ content_type_uid }) => {
    try {
      ...
      const contentTypeResponse = await axios.get<ContentTypeResponse>(
      ...
      const entriesResponse = await axios.get<EntriesResponse>(
...
2. The structure of this content type includes these fields: ${Object.keys(contentType.schema).join(', ')}
